...
C*                            May 21, 2024
C*
C*             *** SEE AERMAP MODEL CHANGE BULLETIN MCB#5 ***
C*
C*     ON THE SUPPORT CENTER FOR REGULATORY AIR MODELS (SCRAM) WEBSITE
C*
C*  https://www.epa.gov/scram/air-quality-dispersion-modeling-related-model-support-programs#aermap
C*
C***********************************************************************
...
C*    Variable Declarations
      ...
      LOGICAL L_temp ! CRT 1/29/2024, for P001 Output Filenames
      CHARACTER*12 MODNAM
      INTEGER IREC, ISRC
c     JAT 03/07/18 variables added for getting
...
C*    Initialize FileExist logical variable
      ...
      L_temp = .FALSE. ! CRT 1/29/2024, for P001 Output Filenames
...
C*    Open the Temporary File for Error Messages Generated from the Program
c      OPEN(UNIT=IERUNT,FILE='ERRMSG.TMP',STATUS='REPLACE') !no longer used as of 18081
...
CCCRT  CRT 4/26/2024 P001 - File already in use
C      Some of the debug files are opened then closed before other
C      Files are opened. Open each debug file. Ensure filenames do not conflict
C      Then close all debug files to be reopened as already programmed.
      INQUIRE(FILE=MAPDET_FILE,OPENED=L_temp)
      IF (L_temp) THEN
         CALL ERRHDL(PATH,MODNAM,'E','505',MAPDET_FILE)
      ELSE
        OPEN(UNIT=DEMK, FILE = MAPDET_FILE, STATUS = 'REPLACE')
      END IF
      INQUIRE(FILE=DOMDET_FILE,OPENED=L_temp)
      IF (L_temp) THEN
         CALL ERRHDL(PATH,MODNAM,'E','505',DOMDET_FILE)
      ELSE  
         OPEN(UNIT=DOMK, FILE=DOMDET_FILE, STATUS='REPLACE')
      END IF
      INQUIRE(FILE=MAPPARAMS_FILE,OPENED=L_temp)
      IF (L_temp) THEN
         CALL ERRHDL(PATH,MODNAM,'E','505',MAPPARAMS_FILE)
      ELSE          
         OPEN (UNIT = MAPK,FILE = MAPPARAMS_FILE,STATUS = 'REPLACE')
      END IF
C     Hill Height Debug
      IF (HILLDBG) THEN
         INQUIRE(FILE=CALCHC_FILE,OPENED=L_temp)
         IF(L_temp) THEN
            CALL ERRHDL(PATH,MODNAM,'E','505',CALCHC_FILE)
         ELSE
            OPEN(UNIT=HDBG,FILE=CALCHC_FILE,STATUS='REPLACE')
         END IF
      END IF 
C     Receptor Debug
      IF (SRCDBG) THEN
         INQUIRE(FILE=RECDET_FILE, OPENED=L_temp)
         IF (L_temp) THEN
            CALL ERRHDL(PATH,MODNAM,'E','505',RECDET_FILE)
         ELSE
            OPEN(UNIT=RECK, FILE=RECDET_FILE, STATUS='REPLACE')
         END IF
         INQUIRE(FILE=RECNDEM_FILE, OPENED=L_temp)
         IF (L_temp) THEN
            CALL ERRHDL(PATH,MODNAM,'E','505',RECNDEM_FILE)
         ELSE
            OPEN(RECD, FILE = RECNDEM_FILE, STATUS = 'REPLACE')
         END IF
         INQUIRE(FILE=RECELV_FILE, OPENED=L_temp)
         IF (L_temp) THEN
            CALL ERRHDL(PATH,MODNAM,'E','505',RECELV_FILE)
         ELSE
...
C     Source Debug
      IF (RECDBG) THEN
         INQUIRE(FILE=SRCDET_FILE, OPENED=L_temp)
         IF (L_temp) THEN
            CALL ERRHDL(PATH,MODNAM,'E','505',SRCDET_FILE)
         ELSE
            OPEN(UNIT=SRCK, FILE=SRCDET_FILE, STATUS='REPLACE')
         END IF
         INQUIRE(FILE=SRCNDEM_FILE, OPENED=L_temp)
         IF (L_temp) THEN
            CALL ERRHDL(PATH,MODNAM,'E','505',SRCNDEM_FILE)
         ELSE
            OPEN(SRCD, FILE = SRCNDEM_FILE, STATUS = 'REPLACE')
         END IF
         INQUIRE(FILE=SRCELV_FILE, OPENED=L_temp)
         IF (L_temp) THEN
            CALL ERRHDL(PATH,MODNAM,'E','505',SRCELV_FILE)
         ELSE
            OPEN (UNIT=SELVK, FILE= SRCELV_FILE, STATUS='REPLACE')
         END IF
      END IF 
C     Close Debug Files
      INQUIRE(UNIT=DOMK,OPENED=L_temp)
      IF (L_temp) CLOSE(DOMK)
      INQUIRE(UNIT=DEMK,OPENED=L_temp)
      IF (L_temp) CLOSE(DEMK)
      INQUIRE(UNIT=MAPK,OPENED=L_temp)
      IF (L_temp) CLOSE(MAPK)
      INQUIRE(UNIT=HDBG, OPENED=L_temp)
      IF (L_temp) CLOSE(HDBG)
      INQUIRE(UNIT=RECK, OPENED=L_temp)
      IF (L_temp) CLOSE(RECK) 
      INQUIRE(UNIT=RECD, OPENED=L_temp)
      IF (L_temp) CLOSE(RECD)
      INQUIRE(UNIT=RELVK, OPENED=L_temp)
      IF (L_temp) CLOSE(RELVK)
      INQUIRE(UNIT=SRCK, OPENED=L_temp)
      IF (L_temp) CLOSE(SRCK)
      INQUIRE(UNIT=SRCD, OPENED=L_temp)
      IF (L_temp) CLOSE(SRCD)
      INQUIRE(UNIT=SELVK, OPENED=L_temp)
      IF (L_temp) CLOSE(SELVK)
...
C        CRT 1/29/2024, P001 Output Filenames - add check to see if hardcoded
C        MAPPARAMS_FILE is already in use
         INQUIRE(FILE=MAPPARAMS_FILE,OPENED=L_temp)
         IF (L_temp) THEN
            CALL ERRHDL(PATH,MODNAM,'E','505',MAPPARAMS_FILE)
         ELSE          
...
CCRT  4/28/2024 P001 - Files already in use
C     Close hill height debug file if open - opened in CALCHC
      INQUIRE(UNIT=HDBG, OPENED=L_temp)
      IF (L_temp) CLOSE(HDBG)
...
C     Variable Declarations
      ...
      CHARACTER*12 MODNAM
      CHARACTER*300 modpre !JAT added 03/07/18. prefix for filenames
      INTEGER LPRE,LINP,I1,LL  !JAT added 03/017/18
...
c     JAT 03/07/18
c     these files will use the output file as the basis for the filename, including
c     directory path of the output file
      modpre=''
      lpre=0
      if (outfil .ne. 'aermap.out') then
         modpre=outfil
         linp=len_trim(outfil)
         lpre=linp
         do i1=linp,1,-1
             if (outfil(i1:i1) .eq. '.' .and. i1 .ne. 1) then
                 modpre=modpre(1:i1-1)
                 lpre=i1-1
                 exit
              endif
      ...
      call uninam( modpre(1:lpre), 'MAPPARAMS.OUT', MAPPARAMS_FILE, ll )
      call uninam( modpre(1:lpre), 'MAPDETAIL.OUT', MAPDET_FILE, ll )
      call uninam( modpre(1:lpre), 'DOMDETAIL.OUT', DOMDET_FILE, ll )
c      MAPPARAMS_FILE = 'MAPPARAMS.OUT'
c      MAPDET_FILE    = 'MAPDETAIL.OUT'
c      DOMDET_FILE    = 'DOMDETAIL.OUT'
...
c      INPFIL = ' ' !commented out JAT 03/13/208
c      OUTFIL = ' ' !commented out JAT 03/13/208
      RECFIL = ' ' 
      SRCFIL = ' '
...
c     JAT 03/07/18: modified open statements to
c     no longer use hard-coded names
C*    OPEN Input Runstream File, Unit INUNIT=5
      DUMMY = 'RUN-STRM'
c      OPEN (UNIT=INUNIT,FILE='aermap.inp',
      OPEN (UNIT=INUNIT,FILE=INPFIL,
     &      ACTION='READ',ERR=99,STATUS='OLD')

C*    OPEN Print Output File, Unit IOUNIT=7
      DUMMY = 'OUTPUT'
C      OPEN (UNIT=IOUNIT,FILE='aermap.out',
...
C*    P010 WSP 2/27/2024 Starting
      IF (ICSTAT(20).NE.1 .OR. IOSTAT(20).NE.1) THEN 
C*      IF (ICSTAT(20).NE.1 .OR. IRSTAT(20).NE.1 .OR. 
C*     &                         IOSTAT(20).NE.1) THEN
C*    P010 WSP 2/27/2024 Ending
...
C           WRITE Error Message: Starting Out of Order
         ...
         ELSE IF (IPNUM .NE. IPPNUM+1) THEN
            IF (PATH .EQ. 'SO' .AND. PPATH.EQ.'RE') THEN  !P010 WSP 3/5/2024 Added     
C*            IF (PATH.NE.'RE' .AND. PPATH.NE.'CO') THEN  !P010 WSP 3/5/2024 Commented out
...
C*    Variable Declarations
      ...
      CHARACTER*12 MODNAM
      CHARACTER modpre*300,str(2)*300
      INTEGER I,I1,LINP
      INTEGER ISLASH,IDOT !JAT added 03/07/18
      LOGICAL FOUND
      INTEGER IOERRN
...
C     JAT 03/07/18
C     create suffix for direct access files based on 
C     input file string prefix
     
      if (trim(adjustl(inpfil)) .eq. 'aermap.inp') then
          str(1)='dir'
          str(2)='idx'
      else
          idot=0
          islash=0
          linp=len_trim(inpfil)
          do i1=linp,1,-1
              if (inpfil(i1:i1) .eq. '.' .and. i1 .ne. 1 .and. 
     +        idot .eq. 0) idot=i1
              if ((inpfil(i1:i1) .eq. '/' .or. inpfil(i1:i1) .eq. '\') 
     +        .and. islash .eq. 0) 
     +        islash=i1
          enddo
          modpre=inpfil(islash+1:idot-1)
          str(1)=trim(adjustl(modpre))//'.dir'
          str(2)=trim(adjustl(modpre))//'.idx'
      endif
...
c         DIRFIL(IDEM) = DEMFIL(IDEM)(1:LOCE(3)-LOCB(3)-2)//'dir'
c         IDXFIL(IDEM) = DEMFIL(IDEM)(1:LOCE(3)-LOCB(3)-2)//'idx'
         DIRFIL(IDEM) = DEMFIL(IDEM)(1:LOCE(3)-LOCB(3)-2)//
     +    trim(adjustl(str(1)))
         IDXFIL(IDEM) = DEMFIL(IDEM)(1:LOCE(3)-LOCB(3)-2)//
     +    trim(adjustl(str(2)))
      ELSE
...
c      DOUBLE PRECISION :: ZFLG  ! P008 CRT 3/20/2024, moved to main module
      INTEGER I, IDUM

C     Variable Initializations
      MODNAM = 'FLAGDF'
      FLGPOL = .TRUE.
C      L_FLGDF = .TRUE. !P008 WSP 1/23/2024
      L_FLGDF = .FALSE. !CRT 4/18/2024 P008 Initialize to False though already initialized in VARINI
      IF (IFC .EQ. 3) THEN
...
C           CRT 4/18/2024 P008, Set L_FLGDF = T if value provided
            L_FLGDF = .TRUE.
            DO I = 1, NREC
               AZFLAG(I) = ZFLG
            END DO
...
CCRT 4/23/2024 P005_P006_P007 Address LINE, RLINE, and BUOYLINE
C    Add variables for finding four corners of a thick line, equivalent to an AREA source
      DOUBLE PRECISION ldx, ldy ! x and y distances
      DOUBLE PRECISION lx1, ly1, lx2, ly2, lx3, ly3, lx4, ly4, lx5, ly5  !4 corners
      DOUBLE PRECISION lxinit, lyinit, langle ! width, length, angle
      INTEGER nverts

C     Variable Initializations
      FOUND = .FALSE.
      MODNAM = 'SOLOCA'
CCRT  P005_P006_P007 LINE - initialize equivalent area variables
      lx1 = 0.0d0; ly1 = 0.0d0
      lx2 = 0.0d0; ly2 = 0.0d0
      lx3 = 0.0d0; ly3 = 0.0d0
      lx4 = 0.0d0; ly4 = 0.0d0
...
C        Error Message: Not Enough Parameters
         ...
         GO TO 999
CCRT P005_P006_P007 Address LINE, RLINE, and BUOYLINE (IFC = 8-9 required) begin
C      ELSE IF (IFC .GT. 7) THEN
       ELSE IF (IFC .GT. 10) THEN
C        Error Message: Too Many Parameters
         CALL ERRHDL(PATH,MODNAM,'E','202',KEYWRD)
         GO TO 999
      END IF   ! WSP 3/26/2024
...
CWSP P005_P006_P007 Address RLINE, and BUOYLINE (IFC = 8-9 required) begin
CCRT        LINE handled separately below
...
CCRT P005_P006_P007 Check LINE separately from RLINE and BUOYLINE with addition of width param.
...
C                 Error Message: Not Enough Parameters
                  ...
                  GO TO 999
               END IF         
C   Addresses POINT, AREA, AREAPOLY, AREACIRC, OPENPIT, SWPOINT, and VOLUME (IFC = 6-7)
            ELSE
               IF (IFC .GT. 7) THEN                    
C                 Error Message: Too Many Parameters        
                  CALL ERRHDL(PATH,MODNAM,'E','202',KEYWRD) 
...
CWSP P005_P006_P007 Address LINE, RLINE, and BUOYLINE (IFC = 8 required) end 
C     Read In The Data Fields and Assign to Arrays
...
CWSP        P005_P006_P007 Address RLINE and BUOYLINE (begin)  
C           Line handled separately below    
C  Read in X1 Y1 X2 Y2, find midpoint for elevation
            ELSE IF (SRCTYP(ISRC).EQ.'RLINE' .OR.        
     &         SRCTYP(ISRC).EQ.'BUOYLINE') THEN     
               CALL STODBL(FIELD(5), 40, AX1S(ISRC), IMIT)        
               CALL STODBL(FIELD(7), 40, AX2S(ISRC), IMIT)         
C              Check The Numerical Field                         
               IF (IMIT .NE. 1) THEN                             
                  CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)      
               END IF                                            
               CALL STODBL(FIELD(6), 40, AY1S(ISRC), IMIT)        
               CALL STODBL(FIELD(8), 40, AY2S(ISRC), IMIT)        
C              Check The Numerical Field                         
               IF (IMIT .NE. 1) THEN                             
                  CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)      
               END IF                                            
C              Find the midpoint of the line to determine elevation later: 
               AXS(ISRC) = (AX1S(ISRC) + AX2S(ISRC))/2           
               AYS(ISRC) = (AY1S(ISRC) + AY2S(ISRC))/2           
               IF (IFC .EQ. 9) THEN                              
C                 Retrieve Source Elevation From Inputs          
                  CALL STODBL(FIELD(9), 40, AZS(ISRC), IMIT)     
C                 Check The Numerical Field                      
                  IF (IMIT .NE. 1) THEN                          
                     CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)   
                  END IF                                         
...
CCRT        P005_P006_P007 LINE -
C           Read in X1 Y1 X2 Y2,
C           Compute corner coordinates for equivalent AREA source 
C           Code taken from AERMOD - duplicates order of coordinates from AERMOD 
            ELSEIF (SRCTYP(ISRC).EQ.'LINE') THEN   
               CALL STODBL(FIELD(5), 40, AX1S(ISRC), IMIT)        
               CALL STODBL(FIELD(7), 40, AX2S(ISRC), IMIT)         
C              Check The Numerical Field                         
               IF (IMIT .NE. 1) THEN                             
               ...
               IF (IFC .GE. 9) THEN                              
C                 Retrieve line width (meters)         
                  CALL STODBL(FIELD(9), 40, AWS(ISRC), IMIT) 
...
C                 Find the four coordinates of the equivalent AREA source
                  ldx = AX2S(ISRC)- AX1S(ISRC)
                  ldy = AY2S(ISRC)- AY1S(ISRC)
                  
                  AXS(ISRC) = (AX1S(ISRC) + AX2S(ISRC))*0.5D0           
                  AYS(ISRC) = (AY1S(ISRC) + AY2S(ISRC))*0.5D0
                  
                  lxinit = AWS(ISRC)
                  lyinit = DSQRT( ldx**2 + ldy**2 )
                  langle = DATAN2(ldx,ldy) * RTODEG
...
C                 Clockwise.  The First Vertex is Repeated as the Last Vertex.
                  lx1 = AX1S(ISRC) - (lxinit/2.0D0)*DCOS(langle*DTORAD)
                  ly1 = AY1S(ISRC) + (lxinit/2.0D0)*DSIN(langle*DTORAD)
     
                  lx2 = lx1 + (lyinit*DSIN(langle*DTORAD))
                  ly2 = ly1 + (lyinit*DCOS(langle*DTORAD))

                  
                  lx3 = lx2 + (lxinit*DCOS(langle*DTORAD))
                  ly3 = ly2 - (lxinit*DSIN(langle*DTORAD))
                  
                  lx4 = lx3 - (lyinit*DSIN(langle*DTORAD))
                  ly4 = ly3 - (lyinit*DCOS(langle*DTORAD))
                  
                  AXS(ISRC) = lx1; AYS(ISRC) = ly1
               
               END IF 
              
               IF (IFC .EQ. 10) THEN                              
C                 Retrieve Source Elevation From Inputs          
                  CALL STODBL(FIELD(10), 40, AZS(ISRC), IMIT)     
C                 Check The Numerical Field                      
                  IF (IMIT .NE. 1) THEN                          
                     CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)   
                  END IF 
...
C                   Write Warning Message for No Source Elevation with PROVIDED option
                  ...
                  END IF
               END IF
CCRT           End Compute corner coordinates P005_P006_P007 LINE               
CWSP        P005_P006_P007 Address LINE, RLINE, and BUOYLINE (end)
...
C        CRT 3/22/2024 should already be initialized
C            set to true if FLGDF is called earlier
CCRT         L_FLGDF = .FALSE. ! P008 WSP 1/23/2024
C*       Read in and set the Flagpole Receptor              ---   CALL FLGHGT
C        CRT 3/22/2024 P008 Flagpole Heights
C        Call FLGHGT only if FLAGPOL = true
         IF (FLGPOL) THEN
            CALL FLGHGT
            IFGSET = IFGSET + 1
         END IF
...
CC*       Warning: Flagpole Receptor Inputs Inconsistent With Options
C         IF (.NOT. L_FLGDF) THEN !P008 WSP 1/23/2024 
C            IF (FLGPOL .AND. IFGSET.EQ.0) THEN
C               CALL ERRHDL(PATH,MODNAM,'W','216',NETIDT)
C               IRZF = IRXR
C            ELSE IF (.NOT.FLGPOL .AND. IFGSET.NE.0) THEN
C               CALL ERRHDL(PATH,MODNAM,'W','215',NETIDT)
C               IRZF = IRXR
C            ELSE IF (.NOT.FLGPOL .AND. IFGSET.EQ.0) THEN
C               IRZF = IRXR
C            END IF
C         ELSE IF (L_FLGDF) THEN !P008 WSP 1/23/2024 
C            CALL ERRHDL(PATH,MODNAM,'W','228',NETIDT) !P008 WSP 1/23/2024 
C            IRZF = IRXR !P008 WSP 1/23/2024 
C         END IF
CCRT     4/18/2024 P008 Flagpole receptor heights
C          Simplify - only two cases will get warnings
C          1) FLAGPOLE=T, L_FLGDF=F, IFGSET=0
C          2) FLAGPOLE=F, L_FLGDF=F, IFGSET>0         
C*       Warning: Flagpole Receptor Inputs Inconsistent With Options

         IF (FLGPOL .AND. .NOT. L_FLGDF .AND. IFGSET.EQ.0) THEN
...
C     Receptor elev and flag hts are supposed to be provided
      IF (.NOT.EXTRACT .AND. FLGPOL) THEN
C        CRT 3/20/2024 P008 Flagpole Heights
C            Should read elevation and flagpole height if both provided.
C            If 5 fields, read field 5 as elevation and use default flagpole height.
C            If 6 or more fields, read field 5 as elevation and field 6 as flagpole height.
C            If fewer than 5 fields, use defaults for elevation (0) and flagpole height.

C         IF (IFC .GE. 5) THEN
C        If 5 or more fields - read field 5 as elevation
         IF (IFC .GE. 5) THEN
...
C*          Check The Numerical Field
            IF (INUM .EQ. -1) THEN
               CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
            ELSE
C              Assign field 5 as receptor elevation            
               AZELEV(I3 + 1) = DNUM
            END IF
C           If 6 or more fields, read field 6 as flagpole height
            IF (IFC .GE. 6) THEN            
...
C                   Assign field 5 as receptor elevation            
         ...
         IF (IFC .EQ. 5) THEN
C           Set flagpole height to default
            AZFLAG(I4 + 1) = ZFLG  
         ELSE IF (IFC .LT. 5) THEN
C           Set elevation to default = 0.0D0
            AZELEV(I3 + 1) = 0.0D0
C           Set flagpole height to default
            AZFLAG(I4 + 1) = ZFLG  
         END IF
CC        Field 6 read as flag height         
C         IF (IFC .GE. 6) THEN
C            CALL STODBL(FIELD(6),ILEN_FLD,DNUM,INUM)
CC*          Check The Numerical Field
C            IF (INUM .EQ. -1) THEN
...
C            END IF
C         END IF
C        CRT 3/20/2024 - A008 Flagpole heights
C            Hill heights are not accepted as input in AERMAP
C         IF (IFC .EQ. 7) THEN
C            CALL STODBL(FIELD(7),ILEN_FLD,DNUM,INUM)
CC*          Check The Numerical Field
C            IF (INUM .EQ. -1) THEN
C               CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
C            ELSE
...
C     Receptor elev should be provided but not flag heights
      ELSE IF (.NOT.EXTRACT .AND. .NOT.FLGPOL) THEN
C        Should read elevation ignore flagpole height if provided.
C        Flagpole height should not be output
C        If 5 or more fields, read field 5 as elevation and and ignore remaining fields.
C        If fewer than 5 fields, use defaults for elevation (0).

C        Field 5 is read as elevation
         IF (IFC .GE. 5) THEN
            CALL STODBL(FIELD(5),ILEN_FLD,DNUM,INUM)
C*          Check The Numerical Field
            IF (INUM .EQ. -1) THEN
...
C     Flagpole heights should be provided, extract elevations
      ELSE IF (FLGPOL .AND. EXTRACT) THEN  
C        CRT 3/20/2024 - A008 Flagpole Heights
C        If 5 fields, read field 5 as flagpole height.
C        If 6 or more fields, read field 6 as flagpole height, ignore field 5.
C        If fewer than 5 fields, use default for flagpole.    
  
C        If only 5 fields, read field 5 as flagpole height        
         IF (IFC .EQ. 5) THEN
            CALL STODBL(FIELD(5),ILEN_FLD,DNUM,INUM)
C*          Check The Numerical Field
            IF (INUM .EQ. -1) THEN
...
C*          Check The Numerical Field
            ...
            ELSE
               AZFLAG(I4 + 1) = DNUM
            END IF
C        Else, if fewer than 5, use flagpole default
         ELSE
            IF (L_FLGDF) THEN
               AZFLAG(I4 + 1) = ZFLG
            ELSE
...
C     Check The Numerical Field
      ...
      ELSE
         AZFLAG(I4 + 1) = DNUM
      END IF
C*     P008 WSP 2/21/2024 Beginning      
C*     Check if FLAGPOLE exists on CO pathway 
      IF (.NOT. FLGPOL) THEN
C*        WRITE Warning Message: 'FLAG Input Inconsistent With Option: Defaults Used'
         CALL ERRHDL(PATH,MODNAM,'W','216','ZFLAG=0.')
      END IF
C*     P008 WSP 2/21/2024 Ending
...
C*          WRITE Error Message: Repeat Non-repeatable Keyword
            CALL ERRHDL(PATH,MODNAM,'E','135',KEYWRD)
C*       P010 Bug Fix WSP 3/5/2024 beginning
CCRT     P010 - correction for error message called - new message added
         ELSE IF (NUMREC .EQ. 0) THEN 
C*          WRITE Error Message: No receptors specified for RECEPTOR output
            CALL ERRHDL(PATH,MODNAM,'E','192','        ')
C*       P010 Bug Fix WSP 3/5/2024 Ending
         ELSE
...
CWSP      P010  Comment out error now that AERMAP can run for sources only
C         IF (IOSTAT(2) .EQ. 0) THEN 
C            print *, '6016'
C            CALL ERRHDL(PATH,MODNAM,'E','185','NUMREC=0') 
C        END IF 
         
CCRT     4/8/2024 P010 Warning message for RECEPTOR keyword but no recs defined
C*       Issue warning for RECEPTOR keyword with EXTRACTED elevations         
         IF (IOSTAT(2).EQ.0 .AND. NUMREC.GT.0 .AND. EXTRACT) THEN
            WRITE(DUMMY,'(I8)') NUMREC
            CALL ERRHDL(PATH,MODNAM,'W','193',DUMMY)
         END IF
...
CCRT     4/8/2024 P010 Error message if neither SOURCLOC or RECEPTOR keywords
C        are not included on the OU pathway - at least one is required.
         IF (IOSTAT(2).EQ.0 .AND. IOSTAT(3) .EQ. 0 .AND. EXTRACT) THEN
            WRITE(DUMMY,'(I8)') NUMSRC
            CALL ERRHDL(PATH,MODNAM,'E','194',DUMMY)
         END IF
...
C     Variable Declarations
      ...
      LOGICAL :: L_temp   !WSP added P001 Output filename bugfix
...
CWSP P001 Output filename bugfix (Begin)
CWSP        OPEN(UNIT=IRUNIT,FILE=RECFIL,ERR=99,STATUS='REPLACE')

            INQUIRE(FILE=RECFIL,OPENED=L_temp)
            IF (L_temp) THEN 
C*             WRITE Error Message:  Filename is Already Opened
CCRT P001 - 1/29/2024 CRT update to pass RECFIL name so more of the filename will print.
C               WRITE(DUMMY,'(A8)') RECFIL(1:8)
               CALL ERRHDL(PATH,MODNAM,'E','505',RECFIL)
            ELSE
               OPEN(UNIT=IRUNIT,FILE=RECFIL,ERR=99,STATUS='REPLACE')
            END IF
CWSP P001 Output filename bugfix (End)
         ELSE
C*          WRITE Error Message:  Filename Field is Too Long
            WRITE(DUMMY,'(I8)') ILEN_FLD
            CALL ERRHDL(PATH,MODNAM,'E','210',DUMMY)
...
C     Variable Declarations
      ...
      IMPLICIT NONE
      LOGICAL :: L_OPEN   !WSP added P001 Output filename bugfix
...
CWSP P001 Output filename bugfix (Begin)
CWSP           OPEN(UNIT=ISUNIT,FILE=SRCFIL,ERR=99,STATUS='REPLACE')

            INQUIRE(FILE=SRCFIL,OPENED=L_OPEN)
            IF (L_OPEN) THEN 
C*             WRITE Error Message:  Filename is Already Opened
C               WRITE(DUMMY,'(A8)') SRCFIL(1:8)
C               CALL ERRHDL(PATH,MODNAM,'E','505',DUMMY)
CCRT P001 - 1/29/2024 CRT update to pass SRCFIL name so more of the filename will print.
                CALL ERRHDL(PATH,MODNAM,'E','505',SRCFIL)
            ELSE
...
9252     FORMAT ('SO ELEVUNIT METERS')
         ...
         DO ISRC = 1, NUMSRC
CWSP P005_P006_P007 Address LINE, RLINE, and BUOYLINE
CWSP Write X1 Y1 X2 Y2
          IF (SRCTYP(ISRC) .EQ.'LINE' .OR.       
     &          SRCTYP(ISRC).EQ.'RLINE' .OR.        
     &          SRCTYP(ISRC).EQ.'BUOYLINE') THEN                        
            WRITE (ISUNIT,9253,ERR=99) SRCID(ISRC), SRCTYP(ISRC), 
     &                          AX1S(ISRC), AY1S(ISRC), AX2S(ISRC),
     &                          AY2S(ISRC), AZS(ISRC)             
9253        FORMAT('SO LOCATION  ',A12,2X,A8,5(2X,F12.2))         
          ELSE                                                    
CWSP P005_P006_P007 Address LINE, RLINE, and BUOYLINE (end) 
...
C         Write out flagpole height if FLAGPOLE included on CO pathway
      ELSE IF (KTYPE .EQ. 'FLAG') THEN
CCRT     4/18/2024 P008 Flagpole Heights - remove code to echo input file format
C        Flagpole heights are written in WRITRZ in same format as terrain and
C        elev and hill heights.  Original comment below indicates not to echo here.
C*       Don't echo runstream inputs for FLAG
C         IF (FLGPOL .AND. IFGSET .GT. 0) THEN
CC*          Write Card to Output File
C            WRITE (IRUNIT,'(a)') RUNST1(1:LEN_TRIM(RUNST1))
C         END IF
...
C      IF (FLGPOL .AND. L_FLGDF) THEN ! P008 WSP 1/18/2024 Starting
      IF (FLGPOL) THEN 
C*     Output DEFAULT FLAGS
C*     Loop through radials/rows and then through # of lines per radial/row
         DO J = 1, IRYT
            DO ILN = 1, NLIN
               IF (ILN .EQ. NLIN .AND. LEFTOV .NE. 0) THEN
               ...
               END IF
               IFIN = IBEG + NITM -1
               WRITE (IRUNIT,3015) KEYWRD,NETIDT,J, 
     &           (AZFLAG(I),I=IBEG,IFIN)
            END DO
         END DO
      END IF ! P008 WSP 1/18/2024 Ending
      
      
 3015 FORMAT(3X,A8,1X,A8,1X,'FLAG ',I4,6(1X,F8.1))  !P008 WSP 1/23/2024
 3016 FORMAT(3X,A8,1X,A8,1X,'ELEV ',I4,6(1X,F8.1))
 3017 FORMAT(3X,A8,1X,A8,1X,'HILL ',I4,6(1X,F8.1))
...
C     Variable Initializations
      ...
      ALLOCATE (AX1S(NSRC), AY1S(NSRC), AX2S(NSRC), AY2S(NSRC)) ! WSP 3/22/2024 P005_P006_P007
      ALLOCATE (AWS(NSRC)) ! CRT 4/3/2024 P005_P006_P007
...
c JAT 03/07/18 subroutine usage added to write out command line argument options
      SUBROUTINE USAGE
C***********************************************************************
C     Variable Declarations
      USE MAIN1
      ...
      WRITE(*,*) ""
      WRITE(*,*) "Usage: AERMAP "//VERSN//" takes either no or one"
     &          // " or two parameters."
      WRITE(*,*) "       Either"
      WRITE(*,*) "             AERMAP"
      WRITE(*,*) "       Or"
      WRITE(*,*) "             AERMAP plumetest.inp"
      WRITE(*,*) "       Or"
      WRITE(*,*) "             AERMAP plumetest.inp plumetest.out"
      WRITE(*,*) ""
...
c     JAT added 03/13/2018: used to construct filenames
