...
c     JAT 18081 made RMIN allocatable based on user email and error
c     with gfortran, originally RMIN was set as RMIN(NDEM)
...
c     JAT added 18081, allocate RMIN
...
C*      at slightly greater than 1300 feet below MSL.
      ...
      IF (HILLDBG .AND. IREC .EQ. 1) THEN
         IF (IREC .EQ. 1) THEN
CCRT         4/17/2024 P001 File already in use
C              check and OPEN statement moved to aermap.f
CC*          Determine whether CALCHC debug file is already opened
CCCRT        CRT 4/15/2024 P001 - File already in use - rewrite to add call to 
CC               error handler if already opened
C            INQUIRE(FILE=CALCHC_FILE, OPENED=L_Opened)
CCCRT            IF (.NOT. L_Opened) THEN
C            IF(L_Opened) THEN
C                CALL ERRHDL(PATH,MODNAM,'E','505',CALCHC_FILE)
C            ELSE
