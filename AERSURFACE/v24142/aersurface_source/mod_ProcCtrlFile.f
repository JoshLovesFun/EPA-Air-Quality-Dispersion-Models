...
C     Control variables required to process land cover file
      ...
      integer (kind=4)  :: NLCDYear  ! 1992, 2001, 2006, 2011, 2013, 2016, 2019, 2021  RLM WSP update
      integer (kind=4)  :: ImpYear   ! 2001, 2006, 2011, 2013, 2016, 2019, 2021        RLM WSP update 
      integer (kind=4)  :: CanYear   ! 2001, 2011 through 2021 (not available in 2006) RLM WSP update
...
CRLM O010_FileCheckNLCD_WSP (BEGIN)
C ----------------------------------------------------------------------
C           Check for Impervious and Canopy data files
C ----------------------------------------------------------------------
            IF (.NOT.gotCan .AND. .NOT.gotImp) THEN
C           If neither exist, recommend use of impervious and canopy data
                 CALL ERRHDL(PATH,ModNam,'W','294','DATAFILE',ILINE)
            ELSE IF (gotCan .XOR. gotImp) THEN
C           If only one input, error, both must be input
                 CALL ERRHDL(PATH,ModNam,'W','295','DATAFILE',ILINE)
CRLM O012_DataYearKeyword_WSP (BEGIN)
CRLM Check Impervious and Canopy year match
            ELSE !gotimp and gotcan true
                IF (ImpYear.NE.NLCDYear .OR. ImpYear.NE.CanYear) THEN
                   CALL ERRHDL(PATH,ModNam,'E','296','DATAFILE',ILINE)
                END IF
CRLM O012_DataYearKeyword_WSP (END)            
            END IF
CRLM O010_FileCheckNLCD_WSP (END)
...
CRLM Change to 'E' 155 (ErrorWarningMessages_WSP)
...
CRLM O010_FileCheckNLCD_WSP (BEGIN)
      LOGICAL :: FEXIST                      ! Existance of file
      CHARACTER (LEN=FilNmLen) :: DFILESHORT !Shortened filename for hint output message
CRLM O010_FileCheckNLCD_WSP (END)

CRLM  O012_DataYearKeyword_WSP (BEGIN)
      DOUBLE PRECISION :: DYEAR !read character year with STODBL
      INTEGER,DIMENSION(10) :: VYEAR=(/2021,2019,2016,2013,2011,2008,
     &                                 2006,2004,2001,1992/) !valid NLCD year list
CRLM  O012_DataYearKeyword_WSP (END)
...
CRLM Check if file was input (E209) RLM WSP 01232024 (Begin)
            IF (LEN(TRIM(DFILE)) .EQ. 0) THEN
                CALL ERRHDL(PATH,ModNam,'E','203',KEYWRD,ILINE)    
            END IF
CRLM Check if file was input (E209) RLM WSP (End)            

CRLM O010_FileCheckNLCD_WSP (BEGIN)
CRLM Check that given DATAFILE DFILE exists
CRLM Rebecca Miller WSP 12/07/2023
      INQUIRE(FILE=DFILE,EXIST=FEXIST)
CRLM Error if file does not exist
      IF (.not.FEXIST) THEN
         DFILESHORT = ADJUSTR(DFILE)
         CALL ERRHDL(PATH,ModNam,'E','290',DFILESHORT(181:),ILINE)
      END IF
CRLM O010_FileCheckNLCD_WSP (END)

CRLM O012_DataYearKeyword_WSP (BEGIN)
CRLM Separates data type (e.g., "NLCD" or "MPRV") from the year
CRLM Year is now read in instead of assigned
CRLM Rebecca Miller WSP 12/20/2023

CRLM        Separate and convert year value
            IF (LEN(TRIM(FIELD(3))) .EQ. 8) THEN !RLM 01232024
                IMIT = 1  !initialize error flag for conversion
                DYEAR = 0 !initialize year
                CALL STODBL(FIELD(3)(5:), ILEN_FLD, DYEAR, IMIT)
                DYEAR = INT(DYEAR) !convert from DBL to integer
CRLM        Check for valid numerical input    
                IF (IMIT .NE. 1) THEN !IMIT from subroutine STODBL
                    CALL ERRHDL(PATH,ModNam,'E','245',FIELD(3),ILINE)
                END IF
            ELSE
                CALL ERRHDL(PATH,ModNam,'E','110',FIELD(3),ILINE)
            END IF   
            
CRLM        Input year can't be before 1992 or less than 4-digits          
            IF (DYEAR < 1992) THEN
                CALL ERRHDL(PATH,ModNam,'E','110',FIELD(3),ILINE)
CRLM        Check whether input year is a valid NLCD year            
            ELSE IF (.NOT.ANY(VYEAR.EQ.DYEAR)) THEN
                CALL ERRHDL(PATH,ModNam,'W','297',FIELD(3),ILINE)
            END IF

            SELECT CASE (FIELD(3)(:4))
               CASE("NLCD")
                  LCFile   = DFILE
                  NLCDYear = DYEAR
                  GotLCD   = .true.
...
CRLMC           Assign file name to file variable, set year of data
CRLMC           NLCDYear.
CRLM            SELECT CASE(FIELD(3))
CRLMC              Assign full Data Type based on FIELD(3) 
CRLM               CASE("NLCD1992")
CRLM                  LCFile   = DFILE
CRLM                  NLCDYear = 1992
CRLM                  GotLCD   = .true.
...
CRLM                  GotCan   = .true.
CRLM               CASE DEFAULT
CRLM                  CALL ERRHDL(PATH,ModNam,'E','110',KEYWRD,ILINE)
CRLM            END SELECT
CRLM O012_DataYearKeyword_WSP (END)
...
CRLM Adding VARYZ0, HIGHZ0 and LOWZ0 to AP/NAP keywords         
CRLM            CASE ('AP')
            CASE ('AP','LOWZ0','LOWZO') !Include O and zero
C              Site is at airport
               Airport = .true.
               VaryAP  = .false.
               
CRLM            CASE ('NONAP')
            CASE ('NONAP','HIGHZ0','HIGHZO')
C              Site is not at an airport
               Airport = .false.
               VaryAP  = .false.
         
CRLM            CASE ('VARYAP')
            CASE ('VARYAP','VARYZ0','VARYZO')
C              Site is at airport
               Airport = .false.
               VaryAP  = .true.
...
CRLM O013_AirportFlag_WSP Begin
      CHARACTER(LEN=6),DIMENSION(6) :: APFLAGLIST=(/"AP    ","NONAP ",
     &                                 "HIGHZ0","LOWZ0 ","HIGHZO",
     &                                 "LOWZO "/) !AP keyword flag list
CRLM O013_AirportFlag_WSP END       
            ...
            CASE(1,8,12,16)
               SELECT CASE(VARYAP)
                  CASE(.true.)      
                    ...
                    IF (IFC == 4) THEN
CRLM O013_AirportFlag_WSP Begin                    
CRLM                      IF((FIELD(4)(1:LEN_TRIM(FIELD(4)))).eq."AP" .or. 
CRLM     &                  (FIELD(4)(1:LEN_TRIM(FIELD(4)))).eq."NONAP")THEN
                      IF (ANY(APFLAGLIST.EQ.
     &                   (FIELD(4)(1:LEN_TRIM(FIELD(4)))))) THEN
CRLM O013_AirportFlag_WSP End     
                           DfltSec = .true.
                      ELSE
     ...
     &                              ILINE)
                      ...
                      END IF
CRLM O013_AirportFlag_WSP Begin                      
CRLM                      IF((FIELD(6)(1:LEN_TRIM(FIELD(6)))).ne."AP".and.
CRLM     &                  (FIELD(6)(1:LEN_TRIM(FIELD(6)))).ne."NONAP")THEN
                      IF (.NOT.ANY(APFLAGLIST.EQ.
     &                   (FIELD(6)(1:LEN_TRIM(FIELD(6)))))) THEN
CRLM O013_AirportFlag_WSP END     
                        CALL ERRHDL(PATH,ModNam,'E','160',KEYWRD,ILINE)
                        SectorErr = .true.
                        RETURN
